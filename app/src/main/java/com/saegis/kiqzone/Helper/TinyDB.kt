...
class TinyDB(appContext: Context?) {
    ...
    fun <T> getListObject(key: String?): ArrayList<Products> {
        val gson = Gson()

        val objStrings = getListString(key)
        val playerList = ArrayList<Products>()

        for (jObjString in objStrings) {
            val player = gson.fromJson(jObjString, Products::class.java)
    ...
    fun putListObject(key: String?, playerList: MutableList<Products>) {
        checkForNullKey(key)
        val gson = Gson()
